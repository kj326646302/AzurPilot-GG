...
class GlobeOperation(ActionPointHandler):
    ...
    def handle_zone_pinned(self):
        ...
        if self.is_zone_pinned():
            # Preserve upstream's short (+50, -50) diagonal gesture inside the
            # ZONE_PINNED area. On reDroid only the backend needs replacing:
            # MaaTouch orientation=2 can ignore the drag, while a long reversed
            # ADB swipe opens/moves other layers and leaves the card pinned.
            # These deterministic points are the center placement generated by
            # upstream for area_pad(ZONE_PINNED.area, pad=-80).
            self.device.handle_control_check('PINNED_DISABLE')
            self.device.swipe_adb((483, 449), (533, 399), duration=0.2)
